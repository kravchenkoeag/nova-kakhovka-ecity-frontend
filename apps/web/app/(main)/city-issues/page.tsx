...
async function getCityIssues() {
  const backendUrl = process.env.BACKEND_URL || "http://localhost:8080";
  try {
    const res = await fetch(`${backendUrl}/api/v1/city-issues`, {
      next: { revalidate: 60 },
    });
    if (!res.ok) return [];
...
export default async function CityIssuesPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="mt-2 text-sm text-gray-600">
            Повідомляйте про проблеми інфраструктури та допомагайте робити місто кращим
          </p>
        </div>
      ...
      <div className="mb-6 flex flex-wrap gap-2">
        ...
        <Button variant="outline" size="sm">Дороги</Button>
        <Button variant="outline" size="sm">Освітлення</Button>
        <Button variant="outline" size="sm">Вода</Button>
        <Button variant="outline" size="sm">Електрика</Button>
        <Button variant="outline" size="sm">Сміття</Button>
      </div>

      {issues.length === 0 ? (
        <div className="text-center py-12">
          <AlertCircle className="mx-auto h-12 w-12 text-gray-400" />
          <h3 className="mt-2 text-sm font-medium text-gray-900">Немає проблем</h3>
          <p className="mt-1 text-sm text-gray-500">Почніть з повідомлення про проблему</p>
        </div>
      ) : (
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  <div className="mt-4 flex items-center gap-4 text-sm text-gray-500">
                    ...
                    <span>
                      {format(new Date(issue.created_at), "dd MMMM yyyy", { locale: uk })}
                    </span>
                  </div>
                </div>
              </div>
