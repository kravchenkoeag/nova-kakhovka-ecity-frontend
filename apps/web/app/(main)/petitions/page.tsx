...
async function getPetitions() {
  ...
  try {
    const res = await fetch(`${backendUrl}/api/v1/petitions`, {
      next: { revalidate: 60 },
    });
    if (!res.ok) return [];
...
export default async function PetitionsPage() {
  const [petitions, session] = await Promise.all([
    getPetitions(),
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="mb-6 flex flex-wrap gap-2">
        ...
        <Button variant="outline" size="sm">Інфраструктура</Button>
        <Button variant="outline" size="sm">Соціальні</Button>
        <Button variant="outline" size="sm">Екологія</Button>
      ...
      {petitions.length === 0 ? (
        <div className="text-center py-12">
          ...
          <h3 className="mt-2 text-sm font-medium text-gray-900">Немає петицій</h3>
          <p className="mt-1 text-sm text-gray-500">Почніть з створення нової петиції</p>
        </div>
      ) : (
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  <div className="mt-4 space-y-2">
                    <div className="flex items-center justify-between text-sm">
                      <div className="flex items-center text-gray-600">
                        ...
                        <span>
                          {petition.signature_count || 0} / {petition.required_signatures || 1000} підписів
                        </span>
                      </div>
                      <div className="flex items-center text-gray-600">
                  ...
                  <div className="mt-4 flex items-center text-sm text-gray-500">
                    ...
                    <span>
                      {format(new Date(petition.created_at), "dd MMMM yyyy", { locale: uk })}
                    </span>
                    {petition.end_date && (
                      <>
                        <span className="mx-2">-</span>
                        <span>
                          До {format(new Date(petition.end_date), "dd MMMM yyyy", { locale: uk })}
                        </span>
                      </>
                    )}
                  </div>
